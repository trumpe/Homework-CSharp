...
namespace ConsoleApp2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Jerry.FavoriteSongs = Songs.Where(s => s.Title.StartsWith("B")).ToList();
            Jerry.GetFavoriteSongs();

            Console.WriteLine();

            Maria.FavoriteSongs = Songs.Where(s => (s.Length / 60) >= 6).ToList();       
            Maria.GetFavoriteSongs();

            Console.WriteLine();

            Jane.FavoriteSongs = Songs.Where(s => s.Genre == Genre.Rock).ToList();
            Jane.GetFavoriteSongs();

            Console.WriteLine();

            Stefan.FavoriteSongs = Songs.Where(s => s.Genre == Genre.Hip_Hop && (s.Length / 60) < 3).ToList();
            Stefan.GetFavoriteSongs();

            Console.WriteLine();

            var personsWithMoreSongs = Persons.Where(fan => fan.FavoriteSongs.Count >= 4).ToList();

            Console.WriteLine("Persons with more songs than 4");
            foreach (var item in personsWithMoreSongs)
            {
